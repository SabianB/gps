...
class SQLDatabase
{
    ...
    private function listEnumValues(string $table, string $col): array
    {
    ...
    public function checkEnumValue(string $table, $col, $val): array
    {
        $enum_values = $this->listEnumValues($table, $col);
        if (!in_array($val, $enum_values)) {
            $valids = '[' . implode(',', $enum_values) . ']';
            $this->fatal("The field $col only accepts the next values: $valids", 400);
        }
        return [];
    }

    public function getEnumValues(string $table, string $col): array
    {
        return $this->listEnumValues($table, $col);
    }
